...
export default function CheckoutForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error } = await stripe.confirmPayment({
      ...
      return_url: "http://localhost:3000/confirmationOfPayment",
      },
    });
  ...
  return (
    <form id="payment-form" onSubmit={handleSubmit} className={styles.background}>
      <LinkAuthenticationElement
        id="link-authentication-element"
        onChange={(e) => setEmail(e.target.value)}
      />
      <PaymentElement id="payment-element" options={paymentElementOptions} />
      <button disabled={isLoading || !stripe || !elements} id="submit" className={styles.buttonCheckout}>
        <span id="button-text">
